...
**`userModified` 剩下的可能与为何测不到**：它只由写文件的工具（Edit / Write）产出，
从不出现在 Bash / Read / Task 上。而本语料的 212413 条记录里：

- `entrypoint` **100% 是 `claude-desktop`**，无 CLI、无 IDE
- `permissionMode` 只有 `acceptEdits`(1213) 与 `auto`(1039)，**两者都自动接受编辑**

也就是说**这份语料从未出现过「编辑需人审批」的流程**。剩下最可能的解释是它标记
「用户在审批时改动了 Claude 提出的编辑」——但确认它需要一次带修改能力的交互式审批，
本地无法自动触发。**未确认，不作猜测写入判据。**
...
- **判据依赖英文消息串**，Claude Code 改文案即静默失效。没有结构化字段可替代
  ...
  数字突变时当作信号而非当作事实。
- **`userModified` 的置真条件仍未确认**（见 §3.4）。已排除磁盘陈旧假设；
  剩余假设是「用户在审批时改动了 Claude 提出的编辑」，确认它需要一次带修改能力的
  交互式审批流程，本地无法自动触发。在确认之前不重新引入该 kind。
