...
> **引用请带日期**。除 §1.2 那次用它的解析器离线跑本机 transcript 之外，全部读码所得；本机未装 Pilot（`~/.loongsuite-pilot` 不存在，`~/.claude/settings.json`
...
**但「零截断」说的是取到的字段，不是每条记录都取到了**：被打断时写下的中断记录大多根本不进事件，
本机实测 265 条只进了 5 条（§1.2）。
...
**但有一类人类消息根本进不了事件：轮末的中断记录。** 解析器按 promptId 给人类消息分组，只在同一 promptId 的
下一次模型调用里，把新增的消息作为输入增量带出去（`transcript-parser.mjs:365-460`）；用户 prompt 事件只取该
promptId 的第一条非元数据文本（`transcript-parser.mjs:498-510`）。`[Request interrupted by user]` 恰好排在被打断那一轮
最后一次模型回复之后，下一轮又换了新 promptId，于是它留在内存里，哪条事件都没带上。拿 Pilot 自己的
`parseClaudeTranscript` 离线跑本机 27 个含中断的主会话 transcript：

| 记录 | 原始条数 | 进了事件 |
|---|---:|---:|
| `[Request interrupted by user]` | 234 | 4 |
| `[Request interrupted by user for tool use]` | 31 | 1 |
| 被拒绝的工具调用（`tool_result` 正文是拒绝话术） | 45 | 41 |

工具结果按 tool_use id 单独收（`transcript-parser.mjs:318-324`），不受这个分组影响，所以拒绝记录大多还在，
只是和机器失败一样记成 `error.type=ToolError`（`:1189-1193`）。这是这份清单里**唯一的实测**：跑的是它的解析器，
不是装机运行，也没覆盖子 agent 文件。
...
- **采集范围要有回归钉子，Pilot 守住了一半。** Pilot 装了 `SubagentStop` 而 teamai 没有；Pilot 还有测试钉住提取层——
  ...
  中断记录在主会话里就被静默丢了（§1.2），这种缩水文件级的钉子抓不到。
...
初稿写完后逐条回查代码，**2 处断言被推翻或补全**（第 1–2 条）；同日第二轮独立复核又改了 **8 处**（第 3–10 条）；第三轮对修正再做独立复核，又推翻 **2 处**（第 11–12 条）。
...
| 12 | 同轮：「正文零截断」「采的是完整对话内容」，隐含每条记录都进了事件 | **不全，而且漏的正好是本项目关心的**：解析器按 promptId 分组，轮末的中断记录大多不进事件。拿它的解析器离线跑本机语料，265 条中断记录只进了 5 条；拒绝工具调用的 45 条进了 41 条 | §0 / §1.2 |
...
**这份文档的局限**：除 §1.2 那次离线跑解析器之外全部读码所得，本机未装 Pilot，没有实机数据可对照——所有关于运行时行为的结论
