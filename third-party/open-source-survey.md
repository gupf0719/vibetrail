...
| 人话与注入 | agentsview 的排除清单比我们多三类：「This session is being continued」开头的续接摘要、「Stop hook feedback:」、IDE 标签；排队的 `queued_command` 按时间并进人话 [源码 `classifyClaudeSystemMessage`、`extractQueuedCommand`] | **是**，清单更全 | 借前三类前缀，先在本机语料上核（Stop hook 反馈在 agentDock 会话里应当不少）；排队人话本机语料里从没当过分歧后的第一句，暂不做 |
| token 用量 | ccusage 按 message.id + requestId 去重，同一键留 token 总数更大的那份，结果与读取顺序无关；早期流式记录的计数可能是占位值 [源码 `should_replace_deduped_entry`，issue #866 / #888] | **是**：我们取最后一条 | 借：同一 message.id 留 output 更大的那份。写缓存可再按 `ephemeral_5m` / `ephemeral_1h` 拆，协议没有对应字段，暂不拆 |
