...
| 内置资源 | 2 个 skill（`teamai-share-learnings`、`team-wiki-codebase`）、1 个 agent（`teamai-recall`） |
...
| **Team Improvement** | 让每次执行反哺团队 | beta | `contribute` / `session save` / `digest` / `dashboard` |
...
**适配 10 个 agent harness**：Claude Code、Codex、Cursor、CodeBuddy、WorkBuddy、
OpenCode、OpenClaw、Hermes、DeepSeek Harness、Qoder。
**适配 6 类 git provider**：GitHub、GitLab、GitCode、CNB、TGit、私有 git
（`src/providers/` 下一个目录一个）。
...
| `prompts` | 有真实文本的 user 记录，排除 interrupt / tool_result / meta / sidechain | 真人轮数 |
...
applyPhase2Adjustments()   + 知识缺口加成（recall 一次没命中 / 命中质量低）
                           − git commit 降权（有 commit **且** recall 有命中 → 说明顺利）
```

阈值 20（`score < 20` 才不提示），即**任何单个强信号——一次打断、一次拒绝、一次纠正
——就够触发**。每个会话最多提示一次（`state.hinted`）。
...
延迟提示的 `pendingHintHandler`）。那句注释描述的是更早的架构，**已过时**。
本文第一版照抄了它，审计时才发现——记在这里提醒后来读这份代码的人。
...
不是本次新发现；本次新增的是**量级**与下面那条 `isSidechain` 的观察：
...
**不是** uuid），记录里带的却是**父会话的 `sessionId`**——而 `isSidechain`
在这些文件里根本不出现，**靠目录层级区分，不是靠字段**。
单个 sessionId 最多横跨 **291 个文件**。

teamai 拿不到这 728 个文件：它注册的 hook 只有 `SessionStart` / `Stop` /
`PostToolUse` / `UserPromptSubmit`，**没有 `SubagentStop`**（已 grep 核实），
全仓也没有任何遍历 `subagents/` 的代码路径。
...
不回答「哪个 commit 属于这个会话」。全仓 grep 未见任何把 session id 写进 commit 的路径。
...
| Cursor | `~/.cursor/cli-config.json` 的 `attributeCommitsToAgent` | 尽力而为（已知上游 bug 可能忽略） |
...
⚠️ **这条设计与本项目的 D2 决策方向相反**，是两边最根本的架构分歧，见对比文档 §4。
...
| 事件 | 用途 | 超时 |
|---|---|---|
| `SessionStart` | 触发 `teamai pull` | 15s |
| `Stop` | friction 检查、会话快照 | 15s |
| `PostToolUse`（`*` / `Skill` / `TodoWrite`） | 计数、埋点 | 10s / 10s / 3s |
| `UserPromptSubmit` | prompt 埋点 | 10s |

全部经 `teamai hook-dispatch <event>` 一个入口分发。
...
| 能力 | teamai-cli | 依据 |
|---|---|---|
| commit ↔ session 关联 | ❌ 无 | 全仓无写 session id 进 commit 的路径；`hasGitCommitInSession` 只做时间窗存在性判断 |
| 行级 / 代码归属 | ❌ 无 | — |
| 审计过程留痕 | ❌ 无 | `review-cmd` / `review-store` 审的是**知识库待审条目**（`.teamai/pending-review.jsonl`），不是代码审计 |
| 留痕数据随代码走 | ❌ 反向设计 | `data-directory-layout.md` 的目标就是业务仓零残留 |
