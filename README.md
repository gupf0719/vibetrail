...
09-17 做完 push（`tools/lib/push.mjs`：开会话补做完、每轮答完、关会话时推（D15）、批是 ack 单位、被拒收只隔离那几条，回归 `tools/test-push.sh`）；09-16 K17 之前格式的旧数据按用户定的 U20 ① 挪出 spool、不推；同日装上新版，第一次真推 9,398 条全部收下。
