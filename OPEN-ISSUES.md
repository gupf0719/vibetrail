...
| **K7** | 已知缺陷 | 🟡 | **按停止打断正在跑的工具，被记成「人拒绝了这次调用」**（2026-09-15 请用户按停止实测发现）。desktop 2.1.266 里用户按停止时，还没跑完的工具会得到一条合成的工具结果：正文「The user doesn't want to proceed with this tool use…」、`toolUseResult`「User rejected tool use」、`toolDenialKind`「user-rejected」，后面跟「[Request interrupted by user for tool use]」——与在权限框里点拒绝**逐字一样**（二进制 `createSyntheticErrorMessage`：user_interrupted 且不是插话结束本轮、不是后台取消时就写这一套）。也没有任何 hook 触发（PostToolUseFailure 没来）。于是 diverge-v1 判成 `permission_denied`、映射成 `permission.decision`（decided_by user）+ `turn.end(denied)`，而它实际是打断。精确率 100% 的说法只对「这是人的动作」成立，对「这是拒绝」不成立；历史语料里的人拒有多少其实是按停止，没量。teamai（toolReject）与 Pilot 同样分不开。**能分开的只有一件事**：真正的拒绝之前一定弹过权限框。PermissionRequest hook 在弹框时触发（payload 有 tool_name、tool_input，没有 tool_use_id，可按输入与时间对上本轮的 tool_use）；Notification 的 `permission_prompt` 也在弹框时发。要动：挂上 PermissionRequest（异步、只记事件头），映射层对 user-rejected 的记录看这次调用有没有弹过框——弹过是拒绝，没弹过是打断（发 turn.end(interrupted)、kind 另立）；auto 模式下大部分调用不弹框，那里的 user-rejected 基本都是按停止。先量，再定要不要改判据（涉及 spec/diverge-v1） |
