...
| **G11** | 需求 | 🔴 | **「多个 session 改了多个代码，然后在最后一个 session 提交了 commit，有办法复盘回溯到当时那个 session 么」**（用户原话，2026-09-10），并且要能回到对话里判断是模型错还是人错。**现状**：`Claude-Session` trailer 只答「谁执行了 commit」（[spec §2 / §4.5](spec/trace-v1.md)）；LoongSuite Pilot 全采也补不上——Bash 改的文件只有命令没有结果、人手改的看不到、没有 git 状态（源码核对见 [TODO.md](TODO.md) §3）。**方案**：工具调用前后给工作树拍快照（独立 index + `git write-tree`），post-commit 时把快照串成影子历史跑 `git blame`，逐行归到「会话:工具调用」或 gap（不是任何 agent 工具调用做的）；[experiments/attrib-demo.sh](experiments/attrib-demo.sh) 已跑通，Bash 改的与人手改的都分得出来。**为什么没动**：刚提出，先记录。**要动需要什么**：先在 agentDock 上量两个数——不拍快照、只做 Edit / Write 内容匹配能覆盖多少行，单次快照要多久；再按 TODO.md §9 拆解做。需求原话、方案、验证、限制、拆解都在 [TODO.md](TODO.md) |
