...
| ~~**U14**~~ | 待定决策 | ✅ | ~~turn.end 什么时候发~~ —— **用户 09-15 定：模型答完就算这一轮结束**（「模型完全回答完应该会有标记的吧。它不可能等下一个turn开始才知道结束」「那排查的时候就会缺失最后一个turn」）。标记是 Claude Code 每次 Stop 跑完 hook 写的 `stop_hook_summary`，之前没有拦停反馈即答完；拒绝停下在拒绝处关；打断没有自己的 hook，打断收尾的轮等下一个 hook（DESIGN D7） |
...
**U11**（2026-09-15 定：只在 Stop / SessionEnd / 补做里解析，从本轮开头读）· **U14**（2026-09-15 用户定：模型答完就发 turn.end，认 `stop_hook_summary`）。
