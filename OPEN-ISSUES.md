...
> 随旧方案（仓内投影、`Claude-Session` trailer）退役的条目同日关闭；2026-09-15 D5（不传 transcript 原文件、云端定协议 1.0、30 天够用）关 U3 / U10 / K1，G5 升前置；同日做完第 1 步「提取器扩展与协议映射」，关 G5、立 U11，随后定 U11、立 U12；G11 的方案细节另记在 [TODO.md](TODO.md)，本表只记一句话与状态。
...
| ~~**K7**~~ | 已知缺陷 | ✅ | ~~按停止打断正在跑的工具，被记成「人拒绝了这次调用」~~ —— **已修（09-15，用户：「K7 按 permissionMode 先粗分，挂上 PermissionRequest」）**：挂上 PermissionRequest 记权限框证据；映射层对「The user doesn't want to proceed…」二次判定——弹过框是拒绝，没弹过、子 agent 里的 User rejected tool use、没挂上时 auto / bypassPermissions / dontAsk 模式下的，都判成按停止，发 `turn.end(interrupted)`、kind `interrupt_tool`（DESIGN D9，spec/diverge-v1 §2.3）。两家都分不开（teamai 同时算一次拒绝加一次打断，Pilot 一律 ToolError）。**待实测**：desktop 里 PermissionRequest 会不会触发——要在 default 模式的会话里弹一次框、点一次拒绝 |
| **K8** | 已知缺陷 | 🟡 | **desktop 续接会话时复制的历史，会按新会话再报一遍**（2026-09-15 用 `vibetrail sync` 补采本仓旧会话时发现）。`aeb3a412`（811 行）的开头被原样复制进了 `5919909a`（347 行）与 `b032681b`（317 行），两个都在另一个 worktree 里：记录 uuid、promptId 完全相同，只有会话 id 换了。event_id 按「会话 id + 记录 uuid」算，所以同一次打断、同一次拒绝、同一批轮次在三个会话里各报一遍（本机这次：分歧 11 条里 3 条是复制来的）。回放副本（同一文件里再追加旧记录，DESIGN §4.2）处理的是另一种情况。要动：机器级记下已发过事件的触发记录 uuid 与 promptId，另一个会话里再遇到时当继承的历史——不再发，或者发时带 `parent_session_id` 指回原会话；SessionStart 的 `source` 有 `fork`，可以先量一下这类会话是不是都从 fork 来。演示报告（`experiments/collect-demo/report.jq`）已先按记录 uuid 合并显示 |
...
| **U13** | 待定决策 | 🟢 | **自定义取值待 collector 确认**（2026-09-15 立，实现轮次元数据时定的暂用值）：`turn.end` 的 status——`completed` / `success`（本轮开始后有过 Stop）、`denied` / `denial`（拒绝后停下）、`unknown` / `unknown`（没有证据）、`<错误类型>` / `error`（StopFailure）；`session.end` 的 status 填 `completed` / `success`；`session.start.capabilities` 暂填我们会发的事件类型清单；hook 事件头的类型名 `ext.claude.<事件名的 snake_case>`。协议只规定是 code 型、没给推荐值（`interrupted` 用户 09-15 说过可以直接加）。**先记录、暂不定**，接 collector 前一并确认，可并入第三轮协议意见（DESIGN §4.1） |
