...
> 随旧方案（仓内投影、`Claude-Session` trailer）退役的条目同日关闭；2026-09-15 D5（不传 transcript 原文件、云端定协议 1.0、30 天够用）关 U3 / U10 / K1，G5 升前置；同日做完第 1 步「提取器扩展与协议映射」，关 G5、立 U11；G11 的方案细节另记在 [TODO.md](TODO.md)，本表只记一句话与状态。
...
| **U11** | 待定决策 | 🟡 | **映射每次从文件头重读的代价**（2026-09-15 立）：映射按行号门控、整文件重读（DESIGN §4.2），实测 106 MB / 4.9 万行纯 jq 映射 10.5 s、5.8 MB 0.7 s（C02FM）。UserPromptSubmit 是同步 hook（30 s 上限、stdout 进模型上下文），大文件要不要跳过、只在 Stop 异步做；或改成保存索引状态的真增量（tool_use 索引 + 链 + pending 落 state 文件）。**先记录、暂不定**，挂 hook（TODO 分歧一路第 2 步）动工前定；先数一下本机 40 个主会话里 > 50 MB 的有几个 |
