...
> 随旧方案（仓内投影、`Claude-Session` trailer）退役的条目同日关闭；G11 的方案细节另记在 [TODO.md](TODO.md)，本表只记一句话与状态。
...
| **G7** | 需求 | 🔴 | **hook 采两路数据（全量 + 人机分歧），HTTP push 云端，机器级装一次，被观测仓零写入**——2026-09-14 定稿为 [DESIGN.md](DESIGN.md)（用户原话 §0、方案 §2–§5、决策 D4、验收 §9），拆解在 [TODO.md](TODO.md)，未定项拆成本表 U1–U10。未开工。过程记录（2026-09-09 提出、09-11 定两路、09-14 七次追加，含 teamai / Pilot 对照要点）在 git 历史里 |
...
| **U3** | 待定决策 | 🟡 | **全量一路的格式**：原样存 transcript（DESIGN §2.1 倾向）还是像 Pilot 规范化成事件；若原样存，是否连 `~/.claude/file-history/` 一起（倾向不存，改前整份文件 transcript 的 `originalFile` 已有）。与 U4 绑定：push 目标若是事件模型的 collector，这项就不能自由选 |
| **U4** | 待定决策 | 🟡 | **云端是什么服务、端点与鉴权、push 的 schema、谁能看**。端点没配之前 push 不发，可以最晚定。候选：paas-coding-hook 事件协议 1.0（[意见](third-party/paas-coding-hook-protocol-feedback.md)）——分歧一路能映射到 tool.end / turn.end 的 status（要加 `denied`），全量的原样字节流放不进它，要么另开通道要么规范化成事件 |
