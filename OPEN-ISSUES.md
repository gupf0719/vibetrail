...
## A. 需要在有 agentDock 与语料的机器上重数的数字

本机没有 agentDock 仓，`~/.claude/projects` 只有几个小 transcript，下面这些**都无法重算**，
文档里已用 ⚠️ 标出，此处汇总：

| 位置 | 写的 | 矛盾 |
|---|---|---|
| [DESIGN.md D3](DESIGN.md) | 402 次 `mark-audit.sh` 调用；570 处不带 sha、18 处带 | 570 + 18 = 588 > 402 |
| DESIGN.md D3 可恢复性表 | 10 / 123 / 48 / 123，分母 294 | 合计 304 ≠ 294，百分比合计 103% |
| DESIGN.md §1、D3 标题 vs D3 正文、spec §4.1 | marker 637 个 vs 294 个 | 两个数的关系没记（目录下全部文件 vs `*.audit.done`？） |
| [CAPABILITIES.md §3.3](CAPABILITIES.md) | `permission_denied` 子 agent 53、主会话 39 | 53 + 39 = 92 ≠ §2.2 / spec §3.2 的 90 |

**为什么没动**：改成任何一个数都是编造。这些数字不改变已定结论（不迁移、不能去重），
所以只标不改。

**要动需要什么**：在那台机器上重跑，并**把统计命令与原始输出留档**（这次的原始统计没留，
所以才只能标不能改）：

- `permission_denied` 主 / 子会话：全语料跑 `tools/extract-diverge.jq`，按记录的 `isSidechain`
  分开数（提取器目前还没输出这个字段，见 C2）。
- marker：`find .claude/audit -type f | wc -l` 与 `find .claude/audit -name '*.audit.done' | wc -l`
  各数一次，写明口径。
- `mark-audit.sh` 调用：从 transcript 的 `tool_input.command` 里数，区分「调用次数」与
  「命令原文里带 sha 的处数」两个口径。
