...
| **U14** | 待定决策 | 🟡 | **turn.end 什么时候发**（2026-09-15 立，DESIGN D7）：实现时定为「这一轮确定结束了才发」——下一轮开始、会话结束、同会话恢复、空闲超过 1 小时后的补做，不在 Stop 当场发。理由：别的 Stop hook 拦停时同一轮会接着干活、再来一次 Stop，当场发会丢掉续上那段的 commit 与用量，而协议只许 commits 挂在 turn.end 上。代价：turn.end 晚到一轮，会话的最后一轮要等 SessionEnd 或空闲后的补做。**待用户确认**；另一种做法是 Stop 当场发、接受被拦停的轮丢后半段 |
...
**U11**（2026-09-15 定：只在 Stop / SessionEnd / 补做里解析，从本轮开头读）。
