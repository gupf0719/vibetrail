...
| ~~**U14**~~ | 待定决策 | ✅ | ~~turn.end 什么时候发~~ —— **用户 09-15 定：模型答完就算这一轮结束**（「模型完全回答完应该会有标记的吧。它不可能等下一个turn开始才知道结束」「那排查的时候就会缺失最后一个turn」）。标记是 Claude Code 每次 Stop 跑完 hook 写的 `stop_hook_summary`，之前没有拦停反馈即答完；拒绝停下在拒绝处关；打断没有自己的 hook，工具运行中的打断靠 PostToolUseFailure 的 `is_interrupt`，文字输出中的打断等下一个 hook（DESIGN D7） |
