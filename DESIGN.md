...
| 方式 | 可恢复 | 强度 |
|---|---|---|
| transcript 里带显式 sha 的 `mark-audit.sh` 调用 | **10 / 294（3%）** | 强，直接证据 |
| marker mtime 唯一落入某 session 时间区间 | 123 / 294（42%） | **弱，旁证未验证** |
| 时间区间歧义（同时落入 2-5 个 session） | 48 / 294（16%） | 不可用 |
| 无任何匹配 | 123 / 294（42%） | 无 |

关键数字：transcript 里共 402 次 `mark-audit.sh` 调用（15 个 session），但其中
