...
- **codex-v4 与 init 引导信任（09-17，看完 teamai / Pilot 的 Codex 适配后改，用户定）**：① 子 agent 与 fork 出来的会话文件开头抄了父会话的 meta 与历史（Pilot 照桌面版多 agent 真实记录做的夹具 `tests/fixtures/codex-subagent/`）：只认 id 等于文件名线程 id 的 session_meta，turn id 的 UUIDv7 时刻（不是 UUIDv7 就用这一轮第一条记录的时间）早于自己 session_meta 的轮不发——v3 会把父会话的轮、人话、token 在子 agent / fork 名下再发一遍；② 子 agent 补 `parent_call_id`：父会话 `spawn_agent` 的结果里带子线程 id（源码 `SpawnAgentResult.agent_id`），或 `task_name` 等于子 agent 的 `agent_path`（桌面版多 agent v2），只在唯一对上时认（照 Pilot 的 codex-subagent-linker），先映射主会话再映射子 agent；③ 认 `web_search_call`（状态在调用上，不编耗时）与 `tool_search_call` / `tool_search_output`；④ init 引导信任：选了 Codex、有没信任或改过的条目时，终端里列出命令并提醒「hook 在 Codex 沙箱外运行」，问一次 [y/N]（默认不写），敲 y 或带 `--trust-codex-hooks` 才往 `config.toml` 加我们自己那几张 `[hooks.state]` 表（只加或替换这几张、其余原样；表头写法认不出来就不写；改前备份、写前后核对、保留权限），写完按哈希核一遍，不写就提示去「设置 → 钩子」；不再选 Codex 或卸载时先删哈希对得上的那几张表。Pilot 部署时直接替用户写信任，teamai 明说不写只提醒，这里取中间：用户在 init 里明确同意才写。两家都没处理 Codex 的人拒绝，这一块只能靠实测。回归 56 项（关掉跳过抄来历史的规则时挂 2 条），Claude 的 test-hook-flow 145 项照过。**本机真写 config.toml 的实测等用户在场**（要测：Codex 开着时外部写的信任是否立即生效、应用会不会用旧状态覆盖）。
- **09-17 信任实测与 codex-v5**：用户在终端里跑 init 敲 y，写进 `config.toml` 的 5 个哈希与上午在 Codex 设置里亲手信任时 Codex 自己写的逐字相同；「设置 → 钩子」认了（5 条开着、没有「信任」按钮、加载问题消失），用户打开设置页之后文件也没被应用改写。桌面版没额度了，「发一句话 hook 立即生效」「在设置里改别的选项不覆盖信任表」两项待测。同时发现用户桌面版开着**自动审批**：那个会话的 `turn_context.approvals_reviewer` 有 `auto_review` 23 次、`user` 11 次，审批先交给 guardian 模型审，还另起一个会话记录（`thread_source: guardian_review`，`source.subagent.other: guardian`，一次审批一轮）。codex-v5：`auto_review` 的轮里被拒不判人拒、不标分歧，记 `codex.approvals_reviewer`；guardian 线程自己的 hook（若会触发）不当独立会话采，连空的 state 目录都不建。回归 58 项（关掉这两条规则时挂 4 条）。
- **09-17 下午实测补记与 codex-v6**：① init 在外部写进 `config.toml` 的信任**当场生效**——14:44 写入，14:49 用户发的一句话 UserPromptSubmit 就触发了（Codex 日志 `logs_2.sqlite` 里有 `hook/started`）；用户在 Codex 设置里改了选项再改回去，Codex 重写了 `config.toml`（3419 → 3434 字节），我们的 5 张信任表都还在。② 用户想测人拒绝，但那个会话每轮都是 `approvals_reviewer: auto_review`，审批被 guardian 模型接走、没弹给人；要测人拒绝得先把审批方式改回由人审。③ **按停止打断一轮之后，这个会话的 Stop hook 再没来过**：14:14:22 那轮有轮止快照、读过 rollout；14:14:46 打断；之后 14:15 与 14:49 两轮都有 `task_complete` 却没有轮止快照，读取进度停在 14:14:23，同期 UserPromptSubmit 每次都触发。Codex 日志里看不到异步 Stop 的执行记录，原因未定（Codex 在打断后不再调 Stop，还是调了但进程被结束）。codex-v6 兜底：Codex 的 UserPromptSubmit（本来就在后台跑）顺带把之前没读的轮读掉，打断的轮与 Stop 没来的轮在下一句话提交时就写出；回归 59 项（去掉补读时挂 1 条）。**待额度恢复后复现**：新开一个会话，正常一轮看 Stop → 打断一轮 → 再一轮看 Stop 还来不来。
