...
| 轮次元数据：turn.end | ✅ 2026-09-15 | `tools/map-events.jq` 主会话按 promptId 切轮，**模型答完就发 `turn.end`**：认 Claude Code 写的 `stop_hook_summary`（之前没有拦停反馈即答完，D7），拒绝停下在拒绝处发；status completed / denied / hook_stopped / unknown / error（打断的由分歧一路发，工具运行中打断由 PostToolUseFailure 的 `is_interrupt` 当场触发解析）；用量按 message.id 去重、vcs 与 commits 取 hook 快照；与分歧同一条事件流，`vibetrail-map --no-turns` 只出分歧；DESIGN §4.1、§4.2 |
