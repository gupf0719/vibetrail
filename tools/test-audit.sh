#!/bin/bash
# vibetrail-audit 回归。重点钉两件事：锚与 spec §4 一致；空锚不写记录。
set -uo pipefail
...
# 锚必须与 spec §4 的命令逐字一致
SPEC=$(sed -n '/```bash/,/```/p' "$SELF/../spec/trace-v1.md" | grep -m1 'patch-id')
...
printf '%s' "$F" | "$SELF/vibetrail-audit" record HEAD audit >/dev/null 2>&1
ck "空锚不写记录" "$before" "$(ls .claude/trace/audits/*.jsonl 2>/dev/null | wc -l | tr -d ' ')"
