# vibetrail：人机分歧判据 v1 的实现（规范 spec/diverge-v1.md §2，判据只在这一处维护）。
#
# 本文件是 jq 模块，只有函数定义。两个调用方 include 它：
#   extract-diverge.jq  命令行入口，逐条输出原始命中（{t:"diverge", kind, human, at, turn, sid, branch, call_id?}）
#   map-events.jq       把命中映射成 paas-coding-hook 协议 1.0 事件（DESIGN §4.1）
# ⚠️ jq 1.6 按 **cwd** 找模块，不按 -f 文件所在目录：调用方必须带 `-L <tools 目录>`。
#
...
#         .toolUseResult 了（user_edited_after_agent 已删，见 spec §2.1），这条留给加规则的人。
...
# 铁律七：.message 也是多态的（语料里未见非对象，纯防御）：一律经 msg 取，别直接 .message.content——
#         .message 是字符串时 .message.content 抛错，后果同铁律四。

def msg: if (.message | type) == "object" then .message else {} end;
...
# 本条记录里所有 is_error 块：{call_id, text}。call_id 是块的 tool_use_id——反查前面 tool_use 块的
# name / input 用（G5，映射层做），块没带就是 null。
def errBlocks:
  if .type == "user" and (msg.content | type) == "array"
  then [msg.content[]? | objects | select(.is_error == true) | {call_id: (.tool_use_id // null), text: (.content | errText)}]
  else [] end;
def errTexts: [errBlocks[] | .text];
...
# kind → 块正文谓词。三类 is_error kind 共用，映射层按块逐个套用
def kindPred($kind):
  if   $kind == "permission_denied"     then isHumanDenial
  elif $kind == "classifier_blocked"    then isClassifier
  elif $kind == "permission_infra_fail" then isInfra
  else false end;

# is_error 类 kind 的命中：多带 call_id（第一个命中块的 tool_use_id）。可选字段，不升版本（spec §6）
def hitErr($kind; $human):
  hit($kind; $human) + {call_id: ([errBlocks[] | select(.text | kindPred($kind))][0].call_id)};
...
# 后者**总是**与 permission_denied 同时出现（同一动作、不同记录）；一条 for-tool-use 可以对应
# **多条** permission_denied——并行发出的几个调用一起被拒时，拒绝各一条、打断只一条（09-15 实测 3:1）。
...
def anyText: if (msg.content | type) == "array"
             then [msg.content[]? | objects | select(.type=="text") | (.text // "")]
             elif (msg.content | type) == "string" then [msg.content]
             else [] end;
...
def diverge:
  ...
  ( select(any(errTexts[]; isHumanDenial)) | hitErr("permission_denied"; true) ),

  # ---- 3. classifier_blocked：auto mode 分类器拒的，机器决定 ----
  ( select(any(errTexts[]; isClassifier)) | hitErr("classifier_blocked"; false) ),

  # ---- 4. permission_infra_fail：权限链路本身失败，不是任何人的决定 ----
  ( select(any(errTexts[]; isInfra)) | hitErr("permission_infra_fail"; false) );

# （曾有第 5 条 user_edited_after_agent，读 .toolUseResult.userModified。已删：desktop 客户端里
#   该字段不可能为真，见 spec §5。fixtures 里 n6-n8 / t9-t10 保留，继续守着「不读多态字段」。）
